...
class BaseLinearStateParams(ABC):
    ...
    def replayssm_ring_bytes_per_req(self, record_len: int) -> int:
        """Bytes of one ReplaySSM spec-verify fold window row (all layers).

        Not part of ``mamba_cache_per_req``, so the memory solver must charge
        it separately: per mamba slot for GDN, per running request for KDA
        (whose window is request-scoped scratch). MUST mirror the ``MambaPool``
        allocation: raw v/k in the conv dtype + fp32 beta, plus the fp32 gate
        ring (per-head scalar for GDN, per-K vector for KDA).
        """
