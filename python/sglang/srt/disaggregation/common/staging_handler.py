...
class DecodeStagingHandler:
    ...
    def register_decode_req(self, room: int, decode_req: DecodeRequest) -> None:
        # Main-thread only; the first registration precedes send_metadata, so
        # no decode_thread activity can exist yet.
        if self._room_to_decode_req.get(room) is decode_req:
            # Re-registration (DecodeTransferQueue.extend): must not wipe
    ...
    def unregister_decode_req(self, room: int) -> None:
        # Pop the room first (atomic) so no new arrival can start consuming
        # its slots; release_room fences the one that may already be past
        # its room lookup.
    ...
    def release_room(self, room: int, decode_req: DecodeRequest, receiver) -> None:
        ...
        # Lock-free by the same discipline as the rest of the handler: the
        # room was popped before this call, so only an arrival already past
        # its lookup can race us. Zero the slots first (best effort: it then
        # skips), and rely on monotonic alloc_ids + idempotent allocator.free
        # for the residual interleavings.
        leaked = []
        for chunk_idx, info in enumerate(chunk_infos):
            if info[0] >= 0:
                leaked.append((chunk_idx, info[0]))
                chunk_infos[chunk_idx] = (-1, -1, 0, -1, 0)
        for chunk_idx, alloc_id in leaked:
            logger.warning(
        ...
        # Recorded scatter kernels may still be reading their regions; sync
        # before freeing.
    ...
    def submit_chunk_scatter(
        ...
        if ok:
            ...
            # Append before zeroing so advance_scatter's lock-free completion
            # check always sees the slot or the event.
