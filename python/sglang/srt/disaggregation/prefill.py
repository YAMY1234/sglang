...
class PrefillBootstrapQueue:
    ...
    def _init_kv_manager(self) -> CommonKVManager:
        ...
        kv_args.draft_total_kv_head_num = 0
        if draft_kv_pool is not None:
            draft_runners = self.scheduler.draft_worker._draft_model_runners()
            if draft_runners:
                kv_args.draft_total_kv_head_num = draft_runners[
                    0
...
class SchedulerDisaggregationPrefillMixin:
    ...
    def process_batch_result_disagg_prefill(
        ...
        # lengths; under PP, batch.spec_info instead holds PPDSparkRelayInput
        # for the aggregate stages and no draft metadata crosses the PD wire.
