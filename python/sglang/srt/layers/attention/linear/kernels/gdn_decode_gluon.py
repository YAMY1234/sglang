...
def packed_step(mixed, gate_a, gate_b, A_log, dt_bias, vbar, fa, fu, fw,
    ...
    # Keep the complete output expression in its original layout. Moving
    # only the final cq product across layouts changes contraction choices.
    oq=l.convert_layout(qn,SliceLayout(0,O))
    ok=l.convert_layout(khat,SliceLayout(0,O))
    ou=l.convert_layout(U,O);ow=l.convert_layout(W,O)
    osr=l.arange(0,16,layout=SliceLayout(1,O))
    oc=l.convert_layout(c,SliceLayout(1,O))
    ocfull=l.where(osr==cnt,clast,oc)
    cq=l.sum(ou*oq[None,:],axis=1)+l.where(osr==cnt,l.sum(ok*oq,axis=0),0.0)
    out=l.convert_layout(out,SliceLayout(0,O))+gt*l.sum(ow*cq[:,None],axis=0)+l.convert_layout(delta,SliceLayout(0,O))*l.sum(ocfull*cq,axis=0)
