...
@triton.jit
def _factor_fp8_io(A, U, W, COUNT, US, WS, BA, BU, BW, BC, IDX, STALE,
...
def transfer(a, u, w, count, us, ws, batch, slots, stale, store=False):
    ba, bu, bw, bc = batch
    # Shapes include L even for a single layer.
    _factor_fp8_io[(slots.numel() * a.shape[2], u.shape[3], a.shape[0])](
        a, u, w, count, us, ws, ba, bu, bw, bc, slots, stale,
        a.shape[1], slots.numel(), a.shape[2], u.shape[3], store, num_warps=4,
    )
