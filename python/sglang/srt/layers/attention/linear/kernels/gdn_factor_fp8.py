...
@triton.jit
def _factor_fp8_decode_io_tiled(A, U, W, COUNT, US, WS, BA, BU, BW, BC, IDX, STALE, LOCAL, BSTALE,
                               S: tl.constexpr, B: tl.constexpr, HV: tl.constexpr, R: tl.constexpr,
                               STORE: tl.constexpr, MAKE_LOCAL: tl.constexpr):
    bh, layer = tl.program_id(0), tl.program_id(1)
    row, h = bh // HV, bh % HV
    slot = tl.load(IDX + row).to(tl.int64)
    local = ((layer * B + row) * HV + h).to(tl.int64)
    glob = (layer * S + tl.maximum(slot, 0)) * HV + h
    col = tl.arange(0, R)
    d = tl.arange(0, 128)
    lo = (local * R + col[:, None]) * 128 + d[None, :]
    go = (glob * R + col[:, None]) * 128 + d[None, :]
    if STORE:
        if slot >= 0:
            u = tl.load(BU + lo).to(tl.float32)
            w = tl.load(BW + lo).to(tl.float32)
            su = tl.maximum(tl.max(tl.abs(u), 1) / 448., 1.e-30)
            sw = tl.maximum(tl.max(tl.abs(w), 1) / 448., 1.e-30)
            tl.store(U + go, (u / su[:, None]).to(tl.float8e4nv).to(tl.uint8, bitcast=True))
            tl.store(W + go, (w / sw[:, None]).to(tl.float8e4nv).to(tl.uint8, bitcast=True))
            tl.store(US + glob * R + col, su)
            tl.store(WS + glob * R + col, sw)
            tl.store(A + glob * 128 + d, tl.load(BA + local * 128 + d))
            tl.store(COUNT + glob, tl.load(BC + local))
        ...
        u = tl.load(U + go).to(tl.float8e4nv, bitcast=True).to(tl.float32)
        w = tl.load(W + go).to(tl.float8e4nv, bitcast=True).to(tl.float32)
        su = tl.load(US + glob * R + col)
        sw = tl.load(WS + glob * R + col)
        tl.store(BU + lo, tl.where(slot >= 0, u * su[:, None], 0.))
        tl.store(BW + lo, tl.where(slot >= 0, w * sw[:, None], 0.))
        tl.store(BA + local * 128 + d, tl.where(slot >= 0, tl.load(A + glob * 128 + d), 0.))
        tl.store(BC + local, tl.where(slot >= 0, tl.load(COUNT + glob), 0))
        if MAKE_LOCAL:
            if layer == 0 and h == 0:
...
def transfer(a, u, w, count, us, ws, batch, slots, stale, store=False, local=None, batch_stale=None, decode=False):
    ...
    kernel = _factor_fp8_decode_io_tiled if decode else _factor_fp8_io
    grid = (slots.numel() * a.shape[2], a.shape[0]) if decode else (slots.numel() * a.shape[2], u.shape[3], a.shape[0])
    kernel[grid](
