...
def truncate_tensor(U, W, count, indices, r, full, *, iters=3, passes=2, extension=None, split=False, precision="ieee", rows=False, lanes=0, whole=False, parallel=False, parallel_lanes=32, chol=0, lu=0, lu_chol=0, panel=False):
    if extension is None:
        from .gdn_jacobi_cuda import load_extension
        extension = load_extension()
    if whole:
        (extension.panel if panel else (extension.parallel if parallel_lanes==32 else getattr(extension,f"parallel{parallel_lanes}")) if (parallel or panel) else ((extension.wholecholp2 if chol==2 else extension.wholechol) if chol else ((extension.wholelup2 if lu==2 else extension.wholelu) if lu else ((extension.wholelucholp2 if lu_chol==2 else extension.wholeluchol) if lu_chol else extension.whole))))(U,W,count,indices,r,full,iters,passes)
        return
    _, h, n, d = U.shape
