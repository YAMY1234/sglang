...
class FusedMoE(torch.nn.Module):
    ...
    def _maybe_load_fp8_expert_as_fp4(
        ...
        """Requantize a block-FP8 expert shard into the MXFP4 expert params.

        Two producers reach here: the fused shared expert of a mixed
        FP8/MXFP4 checkpoint (always FP8 in the file), and every routed expert
        when SGLANG_FP8_MOE_EXPERTS_REQUANT_MXFP4 is set on a plain FP8
        checkpoint. Weight and scale shards arrive as separate tensors, so the
        first of the pair is parked until its partner shows up."""
        if (
