...
class ModelOptNvFp4FusedMoEMethod(FusedMoEMethodBase):
    ...
    def _build_mega_moe_weights(self, layer: torch.nn.Module) -> None:
        """DeepGEMM `nvfp4xnvfp4` MegaMoE layout: e2m1 weights with UE4M3 g16
        scales (packed, gate/up interleaved, UTCCP-transposed) plus the
        per-local-expert alphas the kernel applies in its epilogues. Activations
        are quantized per token at dispatch, so the checkpoint's gate/up
        input_scale is not used; the L2 input keeps its static input_scale."""
        import deep_gemm
        ...
        l2_alphas = (w2_input_scale * layer.w2_weight_scale_2.to(torch.float32)).contiguous()
