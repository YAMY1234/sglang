...
class Scheduler(
    ...
    def _pp_batched_chunk_reuses_req_slot(self, req: Req) -> bool:
        """Whether a waiting middle chunk already owns its request-pool slot."""
        return (
            self.pp_batch_independent_chunks
    ...
    def _pp_order_waiting_queue_for_req_slots(
        ...
        return reusable_reqs + [
            req for req in waiting_queue if req not in reusable_set
        ]
    ...
    def _get_new_batch_prefill_raw(
        ...
        if (
            ...
            and not any(
                self._pp_batched_chunk_reuses_req_slot(req)
                for req in self.waiting_queue
            )
        ):
        ...
        for req in waiting_queue_to_schedule:
            ...
            if (
                not reuses_req_slot
                and new_req_count >= self.get_num_allocatable_reqs(running_bs)
            ):
                running_batch.batch_is_full = True
            if (
                not reuses_req_slot
