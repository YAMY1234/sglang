    ...
    def drain_pending_kv_cache(self) -> int:
        ...
        if self._pending_kv_retirements:
            for req, (
                is_insert,
                last_use_done,
            ) in self._pending_kv_retirements.items():
                last_use_done.synchronize()
                ready.append((req, is_insert))
            self._pending_kv_retirements.clear()
            device = self.token_to_kv_pool_allocator.device
            is_cuda_device = (
                isinstance(device, int) or torch.device(device).type == "cuda"
            )
