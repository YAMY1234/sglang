...
def _pp_exchange_outputs_before_forward(
    ...
    """Decide whether a stage receives the previous outputs before launching.

    A verify round on a non-last stage rebuilds its tree from the relayed
    state and host-syncs on the previous stage's proxy, which that stage only
    sends after its own exchange; launching first would close a wait cycle
    around the ring. An extend microbatch consumes nothing from the relay, so
    it launches first like the non-spec path: exchanging first would make the
    first stage wait for the last stage's result of the microbatch launched
    pp_size - 1 slots earlier, capping every stage at (pp_size - 1) / pp_size
    utilization. The last rank must launch first to produce its output.
    """
