...
class FactoredGDNConfig:
    ...
    def rmax(self) -> int:
        n = self.rfull
        if os.environ.get('SGLANG_GDN_VERIFY_DEFER_CUT', '0') == '1':
            if (self.r, self.m) != (8, 8):
                raise ValueError('deferred verify cut requires r8/W8')
            # Logical capacity 24; Triton rank tiles use a power-of-two pad.
            n = max(n, 24)
        p = 16
        while p < n:
            p *= 2
...
class FactoredGDNPool:
    ...
    def __init__(self, *, size: int, cache_params: BaseLinearStateParams, mamba_layer_ids: List[int], device,
        ...
        if speculative_num_draft_tokens is not None:
            ...
            logger.info('Factored GDN resident rank bucket: %s', os.environ.get('SGLANG_GDN_VERIFY_RESIDENT_RANK_BUCKET', '0') == '1' and getattr(self.spec_state, 'raw_append', False))
            ...
            logger.info('Factored GDN rolled resident verify: %s',
                        os.environ.get('SGLANG_GDN_VERIFY_RESIDENT_LOOP', '0') == '1'
                        and getattr(self.spec_state, 'read_pool', False))
    ...
    def save_prefix_dense(self, layer_id, slots, dense):
        """Publish P after its factors (and optional exact copy) are committed.
