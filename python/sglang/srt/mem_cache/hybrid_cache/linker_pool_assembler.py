...
class DevicePoolEntry:
    ...
    def __init__(
        ...
        *,
        ...
        # Storage object suffix per component group; MooncakeStore falls back to
        # the host-pool temporal/conv naming when this is None.
...
def build_mamba_device_pool_group(
    kvcache: Any, params: Any, page_size: int
) -> DevicePoolGroup:
    """FULL KV pages plus the Mamba state (from req_to_token_pool) as one linker
    group; one checkpoint is one page_size=1 slot keyed by the trailing KV page hash."""
    req_pool = params.req_to_token_pool
    if req_pool.mamba_ckpt_pool is not None:
        raise ValueError(
    ...
    if kvcache.use_mla:
        ...
        # Quantized MHA pools carry per-block scales; MHATokenToKVPool sets both
        # to None otherwise.
        k_scales = full_pool.k_scale_buffer
        v_scales = full_pool.v_scale_buffer
