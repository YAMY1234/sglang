...
class MambaRadixCache(KVCacheEventMixin, BasePrefixCache):
    def __init__(self, params: CacheInitParams):
        ...
        self.mamba_evict_policy = get_server_args().mamba_evict_policy
    ...
    def _enforce_path_state_cap(self, tail: TreeNode) -> None:
        ...
        Fork nodes (children > 1) and locked nodes are skipped: their states
        serve other branches / in-flight requests. KV is kept."""
        holders = []
        node = tail
        ...
        if excess <= 0:
            return
        # holders is ordered tail -> root; trim from the root side.
