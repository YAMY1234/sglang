...
def build_gdn_replayssm_rings(
    *,
    ...
    d = k = rawv = rawk = beta = None
    if not replayssm_spec_fold or cache_params.is_kda:
        d = zeros(*base, hv, L, v_dim, dtype=ring_dtype)
        k = zeros(*base, h_k, L, k_dim, dtype=ring_dtype)
    # The log-decay gate ring (fp32): per-head SCALAR for GDN -> [.., record_len];
    # per-K VECTOR for KDA -> [.., record_len, K].
    if cache_params.is_kda:
        g = zeros(*base, hv, record_len, k_dim, dtype=torch.float32)
    else:
        g = zeros(*base, hv, record_len, dtype=torch.float32)
    ...
    if kda_raw or (enable_linear_replayssm_spec and ring_dtype != torch.float32):
        rawv = zeros(*base, hv, record_len, v_dim, dtype=conv_dtype)
        rawk = zeros(*base, h_k, record_len, k_dim, dtype=conv_dtype)
    if kda_raw:
        beta = zeros(*base, hv, record_len, dtype=torch.float32)
    return ReplaySSMRings(
        d=d, k=k, g=g, rawv=rawv, rawk=rawk, beta=beta, record_len=record_len
    )
