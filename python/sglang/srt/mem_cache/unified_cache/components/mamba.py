...
class MambaComponent(TreeComponent):
    ...
    def update_external_linker_load(
        ...
        *,
        ...
        # The L3 checkpoint landed in the request's private slot; it stays on the
        # tree only until the generic insert has the Full-KV prefix, then detaches.
        lru = self.tree_core.lru_lists[self.component_type]
        if lru.in_list(node):
    ...
    def finalize_external_linker_insert(
        ...
        # Insert raced with an existing local checkpoint, so the remote load is moot;
        # copy that ready checkpoint into the request-owned slot on the forward stream.
