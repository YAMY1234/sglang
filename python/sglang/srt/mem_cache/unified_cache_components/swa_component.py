...
class SWAComponent(TreeComponent):
    ...
    def _has_retained_window_leaf_child(self, node: UnifiedTreeNode) -> bool:
        retain_len = self._window_retention_len()
        cd = node.component_data[self.component_type]
        if len(cd.value) <= retain_len:
            return False
        if len(node.children) != 1:
            return False

        child = next(iter(node.children.values()))
        if child not in self.cache.evictable_device_leaves:
            return False
    ...
    def _drive_eviction_pass(
        ...
        while tracker[ct] < request and x is not None:
            ...
            if x in self.cache.evictable_device_leaves:
                ...
                if (
                    not evict_retained_window
                    and self._has_retained_window_leaf_child(x)
                ):
                    # Insert-time capping commonly leaves an internal prefix
                    # followed by a one-window leaf. Evict the prefix directly:
                    # the leaf is already the retained suffix for this pass.
                    evict_node = x
                else:
                    evict_node = self._window_retention_evict_node(
                        x, evict_retained_window
                    )
