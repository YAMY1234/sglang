...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def _forward_mega_moe(
        ...
        """Routed experts via DeepGEMM MegaMoE; shared expert stays a separate
        replicated MLP (same contract as `_forward_deepep`: rows are this
        rank's tokens, output is fully combined, no TP all-reduce)."""
        from sglang.srt.layers.moe.mega_moe import run_mega_routed_experts

        if not getattr(self.experts, "_mega_moe_weights_built", False):
            raise RuntimeError(
                "moe_a2a_backend=megamoe needs MXFP4 routed experts on this "
                "model: load an MXFP4-expert checkpoint or set "
