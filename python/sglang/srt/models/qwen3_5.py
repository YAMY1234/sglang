...
class Qwen3_5GatedDeltaNet(nn.Module):
    ...
    def forward(
        ...
        if (os.environ.get("SGLANG_GDN_FACTORED_STEP_NORM", "0") == "1"
            ...
            # #756 (ported from the GPT-6 arm): the factored decode step applies this gated RMS norm in-kernel;
            # z is filled in by the backend on the fused projection path. Row blocking follows the local TP heads.
            ...
            decode_norm = (z, self.norm.weight, self.norm.eps,
                           calc_rows_per_block(self.num_v_heads // self.attn_tp_size, self.norm.weight.device),
