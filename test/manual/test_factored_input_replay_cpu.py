...
TRITON_INTERPRET=1 CUDA_VISIBLE_DEVICES='' python test_factored_input_replay_cpu.py
"""
import ast
import copy
import importlib
...
def main():
    ...
    for initial_count in (12, 13, 14, 15):
        ...
        old.spec_state = Old(old, capacity, 4, direct_checkpoints=True)
        new.spec_state = New(new, capacity, 4, qkv_width=width)
        ...
        for iteration in range(21 if initial_count == 12 else 1):
            ...
            for name in Old.names:
                same(getattr(old, name), getattr(new, name), 'checkpoint/replay committed factors')
    ...
    print(json.dumps(dict(passed=True, device='CPU', triton_interpret=True,
        ...
        old_bytes=old.spec_state.bytes(), replay_bytes=new.spec_state.bytes(),
        scope='real GDN verify, checkpoint/accepted replay/sequential states, tracking, W8 all four positions, 17 consecutive zero drafts; not full-model logits or GPU graph admission')))
