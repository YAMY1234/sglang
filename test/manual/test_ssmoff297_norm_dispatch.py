            ...
            for decode in (False,True):
                for batch in (1,3):
                    for rows in (1,4):
                        ...
                        instance=SimpleNamespace(num_v_heads=12,num_k_heads=4,attn_tp_size=2,norm=n,attn=attention,
                        ...
                        def row_layout(m,device):
                            assert m==6, 'normalization rows must use local TP heads'
                            return rows
                        with patch.dict(os.environ,{'SGLANG_GDN_FACTORED_STEP_NORM':str(int(enabled))}), \
                             ...
                             patch.object(norm,'calc_rows_per_block',row_layout):
