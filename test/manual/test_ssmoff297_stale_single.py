"""One per-slot stale publisher preserves all state and output bits."""
                ...
                for count in range(8,16):
                    try:
                        row=decode_case(heads,128 if GPU else 16,batch,count,dtype,
                            qheads=heads//3 if grouped else heads,bias_dtype=bias,stale_single=True)
    ...
    for count in range(8,16):
        try:
            row=decode_case(24 if GPU else 6,128 if GPU else 16,1,count,dtype,
                qheads=8 if GPU else 2,bias_dtype=torch.bfloat16,stale_single=True,near_span=True)
        except Exception as error:
...
if passed and GPU:
    ...
    for enabled in (False,True,True,False):
        torch.manual_seed(297)
        result['timings'].append(captured_step(1,stale_single=enabled))
