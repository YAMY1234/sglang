...
def captured_step(warps, maxnreg=0, gluon=0, early_loads=False, step_pdl=False, expiry_track=False, stale_single=False):
    p=pool(36,24,128)
    p.count.fill_(8)
    ...
    kernels.STEP_STALE_SINGLE=stale_single
    ...
    return dict(warps=warps,maxnreg=maxnreg,gluon=gluon,early_loads=early_loads,step_pdl=step_pdl,expiry_track=expiry_track,stale_single=stale_single,mean_ms=sum(values)/len(values),samples_ms=values,
