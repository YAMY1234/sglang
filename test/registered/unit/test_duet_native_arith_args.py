...
SOURCE = (
    ...
    / "python/sglang/srt/arg_groups/duet_native_arith_args.py"
)
...
class TestDuetComponentParser(unittest.TestCase):
    def test_all_components_are_canonical_immutable_tuple(self):
        self.assertEqual(
            args_module.parse_disabled_components("moe_combine,state,router,rmsnorm"),
    ...
    def test_duplicates_and_whitespace(self):
        self.assertEqual(
            args_module.parse_disabled_components(" state, router ,state "),
            ("router", "state"),
        )
...
@unittest.skipIf(
    STDLIB_ONLY, "explicit --stdlib-only: serving image must run real CLI/runtime tests"
)
class TestDuetNativeArithmeticCLI(unittest.TestCase):
    ...
    def test_opt_in_and_component_ablation_publish_exact_runtime_paths(self):
        raw, features = self.publish(
            "--duet-native-arith",
            "--duet-native-arith-disable-components",
            "state,router,router",
        )
        ...
        self.assertEqual(
            features.duet_native_arith_disable_components, ("router", "state")
        )
        self.assertIsInstance(features.duet_native_arith_disable_components, tuple)
    ...
    def test_moe_combine_disable_reaches_runtime(self):
        ...
        self.assertIs(features.duet_native_arith, True)
        self.assertEqual(
            features.duet_native_arith_disable_components, ("moe_combine",)
        )
